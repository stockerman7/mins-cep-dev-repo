// ExtendScript는 기본적으로 JSON 객체를 지원하지 않는다. 때문에 JSON을 사용하려면 별도의 JSON 라이브러리를 포함
...
function getImagesInFolder(folderPath) {
  ...
  for (var i = 0; i < files.length; i++) {
    ...
    if (file instanceof Folder) {
      images.push({ type: "folder", path: file.fsName, name: decodeURIComponent(file.name) });
    } else if (/\.(jpg|jpeg|png|gif)$/i.test(file.name)) {
      images.push({ type: "file", path: file.fsName, name: decodeURIComponent(file.name) });
    }
  }
